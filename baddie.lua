...
if(getgenv().wcCon ~= nil) then getgenv().wcCon:Disconnect(); getgenv().wcCon = nil end --weather change wc
...
getgenv().wcCon = wc.AttributeChanged:Connect(function(...)
    ...
    if nOver and wOver then getgenv().pauseAutoRejoin = false end 
end)
...
repeat task.wait() until (game:GetService("Players").LocalPlayer and game:GetService("Players").LocalPlayer:FindFirstChild("leaderstats")~= nil and game:GetService("Players").LocalPlayer:FindFirstChild("leaderstats"):GetChildren()[1])
...
--
if Options.AutoWeather.Value and getWeather() ~= nil and Options.Events.Value[getWeather()]  then
    onWeatherFound() --now add detectors for nullity :)
end
